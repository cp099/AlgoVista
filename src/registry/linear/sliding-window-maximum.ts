...
const run: AlgorithmBundle['run'] = function* (inputs) {
    ...
    const makeState = (msg: string, vars: any = {}): AlgoState => ({
        ...
        context: { variables: { k, ...vars }, message: msg }
    });

    yield { snapshot: makeState("Initialized"), events: [], metrics: { comparisons: 0, swaps: 0, writes: 0 } };

    for (let i = 0; i < n; i++) {
        yield { 
            snapshot: makeState(`Processing index ${i} (Value: ${arr[i]})`, { i }), 
        ...
        if (deque.length > 0 && deque[0] <= i - k) {
            ...
            yield { 
                snapshot: makeState(`Index ${removed} is out of window. Popping from Deque front.`, { i }), 
        ...
        while (deque.length > 0 && arr[deque[deque.length - 1]] <= arr[i]) {
            ...
            yield { 
                snapshot: makeState(`arr[${removed}]=${arr[removed]} <= arr[${i}]=${arr[i]}. Popping from Deque back.`, { i }), 
                events: [
        ...
        deque.push(i);
        yield { 
            snapshot: makeState(`Pushing index ${i} to Deque back.`), 
        ...
        if (i >= k - 1) {
            ...
            yield { 
                snapshot: makeState(`Window max is ${arr[maxIdx]}. Pushing to result.`), 
                events: [
